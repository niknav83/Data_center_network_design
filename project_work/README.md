...
3. [Физическая схема текущей сети](#Физическая-схема-текущей-сети-и-краткое-описание-ее-работы)
    - [Краткое описание работы текущей сети](#Краткое-описание-работы-текущей-сети)
    - [Подключение серверов и контроллеров](#Подключение-серверов-и-контроллеров)
    - [Недостатки классической 3-Tier топологии](#Недостатки-классической-3-Tier-топологии)
4. [Выбор новой топологии сети](#Выбор-новой-топологии-сети)
5. [Физическая схема новой сети](#Физическая-схема-новой-сети)
6. [Адресация для построения сети Underlay](#Адресация-для-построения-сети-Underlay)
7. [Адресация для построения сети Overlay](#Адресация-для-построения-сети-Overlay)
8. [Сетевые зоны](#Сетевые-зоны)
9. [Приступаем к настройке сети](#Приступаем-к-настройке-сети)
    - [Настроим интерфейсы, IP адреса и OSPF на всех устройствах Underlay-сети](#Настроим-интерфейсы,IP-адреса-и-OSPF-на-всех-устройствах-Underlay-сети)
    - [Настроим интерфейсы Loopback10, VLAN, VRF на всех устройствах Overlay-сети](#Настроим-интерфейсы-Loopback10,VLAN,VRF-на-всех-устройствах-Overlay-сети)
10. [Приступаем к настройке сети Overlay](#Приступаем-к-настройке-сети-Overlay)
    - [Настроим интерфейсы Loopback10, BGP, VXLAN на всех устройствах Overlay-сети](#Настроим-интерфейсы-Loopback10,BGP,VXLAN-на-всех-устройствах-Overlay-сети)
11. [Приступаем к настройке ESI-LAGs](#Приступаем-к-настройке-ESI-LAGs)
    - [Настроим ESI-LAGs на всех Leaf устройствах Overlay-сети](#Настроим-ESI-LAGs-на-всех-Leaf-устройствах-Overlay-сети)
    - [Настроим LAGs на коммутаторах SW-1-1, SW-1-3, SW-1-5](#Настроим-LAGs-на-коммутаторах-SW-1-1,SW-1-3,SW-1-5)
    - [Настроим интерфейсы на коммутаторах SW-2-1, SW-2-2.](#Настроим-интерфейсы-на-коммутаторах-SW-2-1,SW-2-2.)
12. [Переходим к настройке серверов](#Переходим-к-настройке-серверов)
...
13. [Переходим к проверке работы Undrelay и Overlay сети](#Переходим-к-проверке-работы-Undrelay-и-Overlay-сети)
14. [Файлы конфигураций устройств и сама работа выполненная в EVE-NG](https://github.com/niknav83/Data_center_network_design/tree/main/project_work/configs)

 ## Цель:
- Оптимизация сетевой архитектуры: переход от классической 3-Tier топологии к evpn/vxlan и внедрение функции мультихоминга для эффективного подключения серверов
...
![img_0_0.png](img_0_0.PNG)
  В данной схеме каждый прамоугольник это отдельное здание.

### Краткое описание работы текущей сети.

 - На указанной схеме между коммутаторами обозначенными как Spine-1, Spine-2, Leaf-5 и двумя маршрутизаторами R16 и R14 запущена динамическая маршрутизация реализованная на OSPF.
 - Между двумя коммутаторами Spine-1, Spine-2 два порта ностроены как Link Aggregation Groups (LAG) и они настроены следующим образом:
   ...
   ```
 - На всех коммутаторах сети запущены следующие протоколы:
     - RSTP для блокировки дублирующих линков, где Spine-1 и Spine-2  которые являються корневыми коммутаторами. 
       RSTP настроен на порт, так как часть оборудования не может работать в другом режиме.
     - GARP - Generic Attribute Registration Protocol для автоматического изменения VLAN на trunk портах коммутаторов.

### Подключение серверов и контроллеров. 

- Сервера виртуализации VMWare_vSphere подключены двумя линками сразу к двум коммутаторам так как в них реализован механизм балансировки.
- На серверах с Linux и FreeBSD реализована работа с протоколом  RSTP для этого там были установлены необходимые пакеты.
- Сервера на Windows подключены только одним линком, так как реализовать там дублирование линков пока невозможно в рамках текущей сети.
- Для части оборудования (контроллеров) были подняты Layer-2 GRE Tunnels в связи тем, что они могут между собой взаимодействовать только в рамках L2 сети.

### Недостатки классической 3-Tier топологии.

- При возростании трафика приходится обновлять оборудование или агрегировать линки для обеспечения производительности ядра  сети.  Потому что весь трафик идет через ядро сети.
- При выходе из строя оборудования или линка между ними просходит перестроение stp дерева, что может повлиять на стабильность работы сети или недоступности сегмента или всей сети.


##  Выбор новой топологии сети.

 В связи с тем, что внедрение и переход на L3 топологию сети мог бы решить большую часть проблем, увеличить общую производительности сети, но она не решает задачи L2 связанности для работы устройст и сети виртуализации.

Исходя из вышеизложенного были выбраны следующие технологии:
  -   Построение Underlay сети с использованием OSPF
  -   Построение Overlay  сети с использованием evpn/vxlan
  -   Внедрение функции мультихоминга для эффективного подключения серверов реализованного на базе ESI-LAGs.
    
## Физическая схема новой сети:
...
В данной работе в VRF VoIP и Video для примера добавлено по одному VLAN, в реальной сети их будет несколько для каждого VRF. 
Подключение к интернету в данной схеме будем производить в VRF PROD как маршрут поумолчанию (0.0.0.0 0.0.0.0).
VRF PROD будет использоваться для организации подключения данной схемы к интернету. 
...
## Приступаем к настройке сети Overlay:
...
## Приступаем к настройке ESI-LAGs:
...
Для демонстрации настройки серверов будем использовать образ Mikrotik Router OS версии 7.11.2.
